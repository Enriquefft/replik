...
import { toProductId, type ProductId } from "@/lib/types/ids.ts";
...
export async function createProduct(
  rawInput: unknown,
): Promise<ActionResult<{ id: ProductId }>> {
...
export async function getCreatives(
  ...
  return withUser(userId, async (db) => {
    const creativeRows = await db
      ...
      .from(creatives)
      .where(
        and(eq(creatives.productId, productId), eq(creatives.userId, userId)),
      );
...
export async function selectCreatives(
  ...
  return withUser(userId, async (db) => {
    ...
    for (const creativeId of creativeIds) {
      try {
        await tasks.trigger<typeof translateAndBurnSubsTask>(
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Error al iniciar la edición de video.";
        return {
          ok: false,
