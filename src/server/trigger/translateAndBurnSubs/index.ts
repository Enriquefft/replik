  ...
  run: async (
    ...
    const creative = await withUser(userId, async (db) => {
      const rows = await db
        ...
        .from(creatives)
        .where(
          and(eq(creatives.id, creativeId), eq(creatives.userId, userId)),
        )
        .limit(1);
      return rows[0];
    });
    ...
    const videoUpload = await uploadEditedVideo(
      burned.buffer,
      `${creativeId}.mp4`,
    );
...
async function loadPersistedAssets(
  ...
  const result = await withUser(userId, async (db) => {
    const rows = await db
      ...
      .from(assets)
      .where(
        and(eq(assets.ownerType, "creative"), eq(assets.ownerId, creativeId)),
      );
