...
export const translateAndBurnSubsTask = task({
  ...
  run: async (
    ...
    try {
      ...
      if (!sourceSrtAsset) {
        // Scrape pass persists the SRT alongside `transcript_text`; if it's
        // missing here, the creative pre-dates the scrape-time SRT rollout.
        // Re-running scrape on the product backfills it.
        throw new Error(`source srt asset missing for creative ${creativeId}`)
      }

      // 4. Download the source SRT from UploadThing. Cheap (text/plain),
      // avoids a second Whisper call per burn.
      ...
      const sourceSrt = await srtResponse.text()
      if (sourceSrt.length === 0) {
        throw new Error(`source srt empty for creative ${creativeId}`)
      }
      // Source SRT was generated by Whisper at scrape time; the language
      // detected then is what the SRT timing reflects. `creatives.language`
      // is the SSOT for that detection.
