...
function pickPurchaseCpaCents(
  rows: CostPerActionType[] | undefined,
): number | undefined {
...
function pickPurchaseRoas(
  rows: PurchaseRoas[] | undefined,
): number | undefined {
  if (!rows) return undefined;
  for (const r of rows) {
    if (isPurchase(r.action_type)) {
  ...
  for (const objectId of parsed.ids) {
    ...
    for (const row of body.data ?? []) {
      const insight: Insight = {
        ...
        impressions: Number.isFinite(Number(row.impressions))
