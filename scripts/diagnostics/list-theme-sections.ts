...
const parsed = EncryptedExtraJson.parse(JSON.parse(await decrypt(rows[0].extra)))
...
const key = process.argv[2] ?? "sections/rich-text.liquid"
const fetchAsset = await fetch(
  `https://${shop_domain}/admin/api/2024-10/themes/${main.id}/assets.json?asset[key]=${encodeURIComponent(key)}`,
  { headers: { "X-Shopify-Access-Token": access_token } },
)
const j = await fetchAsset.json()
console.log(j.asset?.value ?? JSON.stringify(j, null, 2))
